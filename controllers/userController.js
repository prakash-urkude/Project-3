...
console.log(data)
        ...
        if(!street || street.trim()=="") return res.status(400).send({status:false,message:"Please provide Street"})
        if(!city || city=="") return res.status(400).send({status:false,message:"Please provide city"})
        if(!pincode || !validatePincode(pincode)) return res.status(400).send({status:false,message:"Please provide valid pincode"})
